package net.cyndeline.scalarlib.rlgraph.regularEdgeLabeling.angularMap
...
class AngularMap[V : TypeTag : ClassTag](rel: EdgeLabeling[V]) {
  ...
  def orientationOfEdge(v1: AngularVertex[V], v2: AngularVertex[V]): AngularVertex[V] = {
    orientations.get(UnorderedPair(v1, v2)).getOrElse {
      throw new Error("The edge " + v1 + " to " + v2 + " is not a part of the angular map.")
    }
  }
